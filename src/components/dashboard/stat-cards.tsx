  ...
  const stats = useMemo((): StatItem[] => {
    if (!now) {
      return [
        ...
        { label: 'Work Completed', value: '...', icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50', borderColor: 'border-emerald-100' },
      ];
    }
        ...
        <Card key={stat.label} className={cn("border bg-card shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl overflow-hidden", stat.borderColor)}>
          <CardContent className="p-6">
            <div className="flex items-start justify-between">
              ...
              <div className={cn("p-3 rounded-xl transition-all group-hover:scale-110 flex items-center justify-center", stat.bg, stat.color)}>
