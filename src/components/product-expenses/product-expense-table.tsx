...
export function ProductExpenseTable() {
  const { productExpenses, deleteProductExpense } = useApp();

  const sortedExpenses = useMemo(() => {
    return [...productExpenses].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [productExpenses]);
  
  const totalExpense = useMemo(() => {
    return productExpenses.reduce((acc, expense) => acc + expense.amount, 0);
  }, [productExpenses]);
            ...
            {sortedExpenses.length === 0 ? (
                ...
                <TableCell colSpan={4} className="h-32 text-center text-muted-foreground">
                    <div className="flex flex-col items-center justify-center space-y-2">
                        ...
                        <p>No product expenses recorded yet.</p>
                    </div>
                </TableCell>
                </TableRow>
            ) : (
            ...
            {sortedExpenses.length > 0 && (
              <TableFooter>
                  <TableRow className="bg-muted/40 font-bold">
                      <TableCell colSpan={2} className="text-right">Total Product Expenses</TableCell>
